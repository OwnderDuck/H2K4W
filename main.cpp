...
void SetLockKey(BYTE virtualKey, bool active) {
    // 检查当前按键状态
    bool isOn = (GetKeyState(virtualKey) & 0x0001) != 0;
    // 如果当前状态与目标状态不符，则模拟按键切换
    if (isOn != active) {
        keybd_event(virtualKey, 0x45, KEYEVENTF_EXTENDEDKEY | 0, 0);
        keybd_event(virtualKey, 0x45, KEYEVENTF_EXTENDEDKEY | KEYEVENTF_KEYUP, 0);
    }
}
void ChangeLockKey(BYTE virtualKey, bool active) {
    keybd_event(virtualKey, 0x45, KEYEVENTF_EXTENDEDKEY | 0, 0);
    keybd_event(virtualKey, 0x45, KEYEVENTF_EXTENDEDKEY | KEYEVENTF_KEYUP, 0);
}
